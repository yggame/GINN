...
class Synapse_dataset_fold(Dataset):
        ...
        if split == "train":
            if dim == 3:
                for file in self.fl_train:
                    data = h5py.File(os.path.join(base_dir, str(file)))
                    img, label = data['image'][:], data['label'][:]
                    data.close()
                    self.example.append((img, label, file, -1))
            else:
                for file in self.fl_train:
                    data = h5py.File(os.path.join(base_dir, str(file)))
                    img, label = data['image'][:], data['label'][:]
                    ...
                    for slice in range(d):
                        self.example.append((img[slice], label[slice], file,  slice))
    ...
    def __getitem__(self, idx):
        ...
        if self.transform and self.dim == 2 and self.split == "train":
            sample = self.transform(sample)
        sample['case_name'] = self.example[idx][2][:8]
        return sample
